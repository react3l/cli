...
export class Command extends Commander {
  ...
  constructor() {
    ...
    this
      .option('-s, --scss', 'Generate component along with SCSS file', false)
      .option('-n, --native', 'Generate React Native component', false)
      .option('-f, --force', 'Force to create contents, overwrite any existing entries', false)
      .command('g:c <name>')
  ...
  public generateWebComponent = (name: string) => {
    ...
    const webComponentRoot: string = NODE_ENV === NodeENV.development
      ? path.join('templates', 'components', 'WebComponent')
      : path.resolve(process.cwd(), 'node_modules', PACKAGE_NAME, 'templates', 'components', 'WebComponent');
    ...
    let componentContent: string = this.renderCommonDataToFile(
      fs
        ...
        .split('WebComponent').join(names.name),
      names,
      gitCredentials,
    );
    if (!this.scss) {
      componentContent = componentContent.split(`import './${names.name}.scss';\n`).join('');
    }
    fs.writeFileSync(
  ...
  public generateNativeComponent = (name: string) => {
    const names: TemplateNames = composeNames(name);
    const gitCredentials: GitCredentials = getGitCredentials();
    const nativeComponentRoot: string = NODE_ENV === NodeENV.development
      ? path.join('templates', 'components', 'NativeComponent')
      : path.resolve(process.cwd(), 'node_modules', PACKAGE_NAME, 'templates', 'components', 'NativeComponent');
    const componentFile = path.join(nativeComponentRoot, 'NativeComponent.tsx');
    const componentSCSSFile = path.join(nativeComponentRoot, 'NativeComponent.scss');
    try {
      fs.mkdirSync(names.name);
    } catch (error) {
      console.log('Can not create directory %s', names.name);
    ...
    let componentContent: string = this.renderCommonDataToFile(
      fs
        ...
        .split('NativeComponent').join(names.name),
      names,
      gitCredentials,
    );
    if (!this.scss) {
      componentContent = componentContent.split(`import styles from './${names.name}.scss';\n`).join('');
    }
    fs.writeFileSync(
      path.join(names.name, `${names.name}.tsx`),
      componentContent,
  ...
  public renderCommonDataToFile = (content: string, names: TemplateNames, gitCredentials: GitCredentials) => {
    const currentTime: string = getCurrentTimeString();
    return content
      .split('<current-time>').join(currentTime)
      .split('<git-name>').join(gitCredentials.name)
      .split('<git-email>').join(gitCredentials.email);
  }
}
